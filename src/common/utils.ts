...
export class GraphQLClient {
  ...
  constructor(baseUrl: string = 'https://api.github.com/graphql') {
    ...
    if (process.env.GITHUB_PERSONAL_ACCESS_TOKEN) {
      this.headers['Authorization'] = `Bearer ${process.env.GITHUB_PERSONAL_ACCESS_TOKEN}`;
    }
  }
  ...
  async request<T = unknown>(query: string, variables?: Record<string, unknown>): Promise<T> {
    ...
    if (!response.ok) {
      throw createGitHubError(response.status, result);
    }
